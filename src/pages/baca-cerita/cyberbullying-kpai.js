...
const CyberbullyingKPAI = () => {
  const [komikCyberbullyingKPAI, setKomikCyberbullyingKPAI] = useState([]);

  useEffect(() => {
    const q = query(
      collection(db, "bullying-kpai"),
      where("id", ">", 0),
      orderBy("id")
    );
    onSnapshot(q, (snapshot) => {
      setKomikCyberbullyingKPAI(
...
export default CyberbullyingKPAI;
